...
const GalleFortTour = () => {
  ...
  const schemaData = {
    ...
    "description": "Rated #1 historical walking tour in UNESCO World Heritage Galle Fort. Expert local guidance.",
    "url": "https://gallefortwalkingtour.com",
    "image": "https://www.galleexperience.com/images/about.webp",
    "provider": { "@type": "LocalBusiness", "name": "Galle Walk", "telephone": "+94776353353" },
  ...
  const handleBooking = async (e) => {
    e.preventDefault();
    if (!user) { const p = new GoogleAuthProvider(); await signInWithPopup(auth, p); return; }
  ...
  return (
    <div className="min-h-screen bg-white text-[#0F172A] font-sans selection:bg-cyan-100 overflow-x-hidden">
        <Helmet>
            <title>Galle Fort Walking Tour | #1 Rated Guide Madawa</title>
            <meta name="description" content="Official walking tour in UNESCO World Heritage Galle Fort. Book with expert resident Madawa. Rated 5.0 on TripAdvisor." />
            <link rel="canonical" href="https://gallefortwalkingtour.com" />
            <script type="application/ld+json">{JSON.stringify(schemaData)}</script>
        </Helmet>

        <style>{`
          .glass-nav { background: rgba(255, 255, 255, 0.7); backdrop-filter: blur(40px); border-bottom: 1px solid rgba(255, 255, 255, 0.1); }
          ...
          .hero-btn { height: 56px; display: inline-flex; align-items: center; justify-content: center; padding: 0 36px; border-radius: 99px; font-weight: 900; text-transform: uppercase; letter-spacing: 0.1em; font-size: 11px; transition: all 0.4s ease; border: 1.5px solid transparent; }
          .btn-elegant { background: linear-gradient(135deg, #0F172A 0%, #1E293B 100%); transition: all 0.4s ease; }
        `}</style>

        <a href={`https://wa.me/${MADAWA_PHONE}`} target="_blank" className="wa-float" aria-label="WhatsApp Madawa">
            ...
            <div className="flex items-center gap-8">
                {user ? (
                    <div className="text-right flex flex-col items-end">
                      ...
                      <button onClick={() => signOut(auth)} className="text-[9px] font-black text-red-500 uppercase mt-1.5">Logout</button>
                    </div>
                ) : (
                  <div className="flex items-center gap-6">
                    <a href={TRIPADVISOR_URL} target="_blank" className="ta-pill hidden lg:flex items-center gap-2 px-5 py-2.5 rounded-full text-white text-[10px] font-black"><Star size={10} fill="currentColor"/> 5.0</a>
                    <button onClick={() => signInWithPopup(auth, new GoogleAuthProvider())} className={`flex items-center gap-2 px-6 md:px-8 py-2 md:py-3 rounded-full text-[9px] font-[900] uppercase tracking-widest transition-all ${scrolled ? 'bg-[#0F172A] text-white shadow-xl' : 'bg-white text-slate-900'}`}><img src={GOOGLE_ICON} className="w-4 h-4 rounded-full" alt="Google icon" />Login</button>
                  </div>
                )}
            </div>
          </nav>
        ...
        <section className="relative h-[100dvh] w-full flex items-center justify-center overflow-hidden bg-[#0F172A]">
            <div className="absolute inset-0 z-0">
                <AnimatePresence initial={false}>
                    <motion.img key={slideIndex} src={BACKGROUND_SLIDES[slideIndex]} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.2 }} className="absolute inset-0 w-full h-full object-cover ken-burns" alt="Galle Fort Walking Experience" />
                </AnimatePresence>
            ...
            <div className="relative z-20 text-center px-6 w-full max-w-7xl mx-auto">
                ...
                <div className="h-[120px] md:h-[200px] flex flex-col justify-center mb-6">
                    <AnimatePresence mode="wait">
                        <motion.div key={textIndex} initial={{ opacity: 0, filter: "blur(20px)", scale: 0.9 }} animate={{ opacity: 1, filter: "blur(0px)", scale: 1 }} exit={{ opacity: 0, filter: "blur(20px)", scale: 1.1 }} transition={{ duration: 1.2 }}>
                            <h2 className="text-4xl sm:text-6xl md:text-[5.4rem] font-[950] text-outline leading-tight uppercase mb-1">{ROTATING_HEADLINES[textIndex].top}</h2>
                ...
                <p className="text-white/80 text-lg md:text-xl font-medium max-w-2xl mx-auto mt-12 leading-relaxed">Step inside the living history of the Dutch Fort with <span className="text-white font-[950]">Madawa</span>, the most recommended resident local guide.</p>
                <div className="mt-14 flex flex-wrap justify-center gap-8">
                    <a href="#book" className="hero-btn bg-[#0891B2] text-white shadow-xl hover:bg-white hover:text-[#0891B2]">Book Walk</a>
                    <a href={TRIPADVISOR_URL} target="_blank" className="hero-btn border-white/20 bg-white/5 backdrop-blur-2xl text-white">Reviews</a>
                </div>
            </div>
        </section>
        ...
        <main className="container mx-auto px-6 py-24 max-w-7xl">
            <div className="grid lg:grid-cols-12 gap-20">
                <section className="lg:col-span-7 space-y-24">
                    <article className="space-y-12">
                        <h2 className="text-3xl font-[950] uppercase tracking-tighter text-slate-900 border-b-[8px] border-[#0891B2] pb-1 inline-block">The Experience</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            {[{ n: "Bill S.", t: "Highly recommended resident guide. Friendly and fun tour!" }, { n: "Hima M.", t: "Knowledgeable. The trip was full of facts and humour." }].map((fb, i) => (
                                <div key={i} className="p-10 bg-slate-50 rounded-[3rem] border border-slate-100/50">
                                    <p className="text-slate-600 italic text-base mb-6 font-medium">"{fb.t}"</p>
                                    <p className="text-[10px] font-[900] uppercase tracking-widest text-[#0891B2] flex items-center gap-3"><Star size={14} fill="currentColor"/> Guest: {fb.n}</p>
                                </div>
                            ))}
                        </div>
                        <a href={TRIPADVISOR_URL} target="_blank" className="flex items-center justify-between p-8 bg-[#00af87] rounded-[2.5rem] text-white shadow-lg group">
                            <div className="flex items-center gap-6"><img src="https://static.tacdn.com/img2/brand_refresh_2025/logos/wordmark.svg" className="h-6 brightness-0 invert" alt="TA" /><span className="font-black text-base md:text-xl underline decoration-white/20 underline-offset-[8px]">250+ Reviews</span></div>
                            <ArrowRight size={24} />
                        </a>
                        <div className="space-y-6">
                            <h3 className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.3em] text-slate-400"><Camera size={16} className="text-[#0891B2]"/> Moments From Our Walks</h3>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                {TOUR_GALLERY.map((img, i) => (<div key={i} className="rounded-2xl overflow-hidden shadow-md"><img src={img} alt="Tour Group" className="w-full h-full object-cover aspect-square" /></div>))}
                            </div>
                        </div>
                    </article>
                    <section ref={bookingsRef}>
                        {user && myBookings.length > 0 && (
                            <div className="pt-10 border-t border-slate-100">
                                <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 mb-8">Active Bookings</h3>
                ...
                <aside id="book" className="lg:col-span-5">
                    <section className="bg-white p-10 md:p-12 rounded-[4rem] shadow-[0_40px_80px_rgba(0,0,0,0.06)] border border-slate-100 sticky top-32">
                        <div className="flex justify-between items-end mb-10"><h2 className="text-2xl font-[950] uppercase">Book Walk</h2><div className="text-right"><span className="text-4xl font-[950]">${bookingType === 'Group Walk' ? settings.groupPrice : settings.privatePrice}</span><p className="text-[9px] font-black uppercase text-slate-400">per person</p></div></div>
                        <form onSubmit={handleBooking} className="space-y-4">
                            <div className="flex p-1 bg-slate-100 rounded-[1.8rem] mb-8">
                                {['Group Walk', 'Private Tour'].map(t => (<button key={t} type="button" onClick={() => setBookingType(t)} className={`flex-1 py-3 rounded-[1.4rem] font-[900] text-[8px] md:text-[9px] uppercase tracking-widest ${bookingType === t ? 'bg-[#0891B2] text-white shadow-xl' : 'text-slate-400'}`}>{t}</button>))}
                            </div>
                            <input required className="bg-slate-50 w-full p-4 rounded-2xl border border-slate-100 text-sm font-bold outline-none focus:bg-white" placeholder="Guest Name" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
                            <div className="relative"><select className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 text-sm font-bold appearance-none" value={form.country} onChange={e => setForm({...form, country: e.target.value})}>{ALL_COUNTRIES.map(c => <option key={c} value={c}>{c}</option>)}</select><ChevronDown size={16} className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-300" /></div>
                            <input required className="bg-slate-50 w-full p-4 rounded-2xl border border-slate-100 text-sm font-bold outline-none" placeholder="WhatsApp Number" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} />
                            <div className="grid grid-cols-2 gap-4"><div className="bg-slate-50 p-4 rounded-xl"><label className="block text-[8px] font-black text-slate-400 uppercase mb-2">Date</label><input required type="date" className="bg-transparent w-full text-xs font-black outline-none" value={form.date} onChange={e => setForm({...form, date: e.target.value})} /></div><div className="bg-slate-50 p-4 rounded-xl"><label className="block text-[8px] font-black text-slate-400 uppercase mb-2">Time</label><select className="bg-transparent w-full text-xs font-black outline-none appearance-none" value={form.time} onChange={e => setForm({...form, time: e.target.value})}>{STARTING_TIMES.map(t => <option key={t} value={t}>{t}</option>)}</select></div></div>
                            <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100"><label className="block text-[10px] font-black text-slate-400 uppercase mb-3">Number of People</label><div className="flex items-center justify-between"><button type="button" onClick={() => setForm(f => ({...f, peopleCount: Math.max(1, f.peopleCount - 1)}))} className="w-10 h-10 rounded-full bg-white border flex items-center justify-center font-bold shadow-sm">-</button><span className="text-xl font-black text-[#0891B2]">{form.peopleCount}</span><button type="button" onClick={() => setForm(f => ({...f, peopleCount: Math.min(20, f.peopleCount + 1)}))} className="w-10 h-10 rounded-full bg-white border flex items-center justify-center font-bold shadow-sm">+</button></div></div>
                            <button disabled={isSubmitting} className="w-full py-5 bg-[#0F172A] text-white rounded-[2rem] font-[900] uppercase text-[10px] shadow-lg mt-10 hover:bg-[#0891B2] transition-all flex items-center justify-center gap-3">
                                {isSubmitting ? <Loader2 className="animate-spin" size={20}/> : user ? "Book Now" : <><img src={GOOGLE_ICON} className="w-4 h-4 rounded-full" alt="Google icon" />Login</>}
                            </button>
                        </form>
                    </section>
                </aside>
        ...
        <section className="container mx-auto px-6 mb-24 max-w-7xl"><div className="bg-[#0891B2] rounded-[4rem] p-12 md:p-20 text-white flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl relative overflow-hidden group"><div className="max-w-xl relative z-10 text-center md:text-left"><h2 className="text-3xl md:text-5xl font-[950] tracking-tighter leading-none mb-6 uppercase">WANT OTHER <br/>EXPERIENCES?</h2><p className="text-cyan-50 text-base md:text-lg italic opacity-90 mb-0">Discover boat safaris and rainforest treks beyond the fort walls.</p></div><a href="https://www.galleexperience.com/" target="_blank" className="bg-white text-[#0891B2] px-10 py-5 rounded-full font-black uppercase text-[11px] tracking-widest shadow-xl flex items-center gap-3">Galle Experience <ExternalLink size={16} /></a></div></section>
        <footer className="bg-[#0F172A] py-16 text-center text-white/20 border-t border-white/5 uppercase text-[10px] tracking-[1.5em] font-black px-4">HISTORY • HERITAGE • GALLE</footer>
